...
process_remedy = function(remedy_raw){
  ...
  remedy_processed = lapply(remedy_processed, function(x) import_string_normalize(x))
...
process_cases = function(remedy){
  ...
  remedy_processed = remedy_processed %>% select(id, call_id, caller_first_name, caller_second_name, assigned_to, caller_type, agent_login_id, ward_no__decoded_from_ward_name_, 
                                                 host_department, call_topic_category_2, details, service_address, call_opened_date, original_create_date, original_create_time, last_modified_by)
  # Normalize address
